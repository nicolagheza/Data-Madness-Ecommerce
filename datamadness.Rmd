...
ggplot() + 
  geom_line(data=revenueByDate, aes(x = date, y = clean_ts)) + labs(x='Date', y='Revenue (£)', title='TS Revenue by Date')
...
customer_spending <- df_data %>%
  group_by(CustomerID) %>%
  summarise(purchases = sum(Quantity*UnitPrice), Country = Country[1])
